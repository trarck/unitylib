...
public static class BezierSpline
{
	...
	public static void GetCurveControlPoints(Vector3[] knots, out Vector3[] firstControlPoints, out Vector3[] secondControlPoints)
	{
		...
		{ // Special case: Bezier curve should be a straight line.
			...
			// 3P1 = 2P0 + P3
			firstControlPoints[0] = (2 * knots[0] + knots[1]) / 3;
		...
		for (int i = 0; i < n; ++i)
		{
			...
			if (i < n - 1)
				secondControlPoints[i] = 2 * knots[i + 1] - firstControlPoints[i + 1];// new Vector3(2 * knots[i + 1] - firstControlPoints[i + 1], 2 * knots[i + 1].Y - y[i + 1]);
			else
				secondControlPoints[i] = (knots[n] + firstControlPoints[n - 1]) * 0.5f;// new Point((knots[n].X + x[n - 1]) / 2, (knots[n].Y + y[n - 1]) / 2);
		}
	}
