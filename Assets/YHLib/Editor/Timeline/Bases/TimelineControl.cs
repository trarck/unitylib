...
namespace YHEditor.Timeline
{
    internal class TimelineControl
    {
        ...
        {
            if (id == -1)
                id = UnityEditorBridge.GetPermanentControlID();
        ...
        private void DoPivotCurves()
        {
            ...
            Handles.DrawAAPolyLine( m_SrcPivotVectors);
            Handles.DrawAAPolyLine( m_DstPivotVectors);
            GUI.EndGroup();
        }
        ...
        public bool DoTimeline(Rect timeRect)
        {
            ...
            GUI.BeginGroup(timeRect);
            {
                ...
                GUIContent srcContent = UnityEditorBridge.TempContent(SrcName);
                ...
                GUIContent dstContent = UnityEditorBridge.TempContent(DstName);
                ...
                if (m_DragState == DragStates.Destination && !oneFrameDst)
                {
                    ...
                    string transLabel = UnityEditorBridge.Format("{0:0%}", (transStart - dstStart) / (dstStop - dstStart));
                    GUI.Box(transLabelRect, UnityEditorBridge.TempContent(transLabel), styles.timeBlockRight);
                }
                ...
                if (m_DragState == DragStates.LeftSelection)
                {
                    if (!oneFrameSrc)
                    {
                        ...
                        string srcLabel = UnityEditorBridge.Format("{0:0%}", (transStart - srcStart) / (srcStop - srcStart));
                        GUI.Box(srcLabelRect, UnityEditorBridge.TempContent(srcLabel), styles.timeBlockRight);
                    }

                    if (!oneFrameDst)
                    {
                        Rect dstLabelRect = new Rect(transStart - 50, dstRect.y, 45, dstRect.height);
                        string dstLabel = UnityEditorBridge.Format("{0:0%}", (transStart - dstStart) / (dstStop - dstStart));
                        GUI.Box(dstLabelRect, UnityEditorBridge.TempContent(dstLabel), styles.timeBlockRight);
                    }
                }
                ...
                if (m_DragState == DragStates.RightSelection)
                {
                    if (!oneFrameSrc)
                    {
                        ...
                        string srcLabel = UnityEditorBridge.Format("{0:0%}", (transStop - srcStart) / (srcStop - srcStart));
                        GUI.Box(srcLabelRect, UnityEditorBridge.TempContent(srcLabel), styles.timeBlockLeft);
                    }
                    ...
                    if (!oneFrameDst)
                    {
                        ...
                        GUI.Box(dstLabelRect, UnityEditorBridge.TempContent(dstLabel), styles.timeBlockLeft);
                    }
                }
