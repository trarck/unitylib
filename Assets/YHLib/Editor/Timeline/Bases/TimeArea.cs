...
namespace YHEditor.Timeline
{
    ...
    class TimeArea : ZoomableArea
    {
        ...
        public void DrawMajorTicks(Rect position, float frameRate)
        {
            ...
            UnityEditorBridge.ApplyWireMaterial();
        ...
        public float TimeField(Rect rect, int id, float time, float frameRate, TimeFormat timeFormat)
        {
            ...
            {
                float newTime = UnityEditorBridge.FloatField(
                    rect,
                    id,
                    time);
            ...
            if (timeFormat == TimeFormat.Frame)
            {
                ...
                int newFrame = UnityEditorBridge.IntField(
                    rect,
                    id,
                    frame);
            ...
            else // if (timeFormat == TimeFormat.TimeFrame)
            {
                ...
                bool changed;
                str = UnityEditorBridge.TextField(rect,id, str,allowedCharacters, out changed);
        ...
        {
            float newValue = UnityEditorBridge.FloatField(
                rect,
                id,
                value);
        ...
        {
            int vDecimals = GetNumberOfDecimalsForMinimumDifference(shownArea.height / drawRect.height);
